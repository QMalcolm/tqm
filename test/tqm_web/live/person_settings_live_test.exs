defmodule TqmWeb.PersonSettingsLiveTest do
  use TqmWeb.ConnCase
  ...
  describe "confirm email" do
    ...
    test "updates the person email once", %{
      ...
      {:error, redirect} = live(conn, ~p"/people/settings/confirm_email/#{token}")
    ...
    test "does not update email with invalid token", %{conn: conn, person: person} do
      {:error, redirect} = live(conn, ~p"/people/settings/confirm_email/oops")
      assert {:live_redirect, %{to: path, flash: flash}} = redirect
      assert path == ~p"/people/settings"
      assert %{"error" => message} = flash
      assert message == "Email change link is invalid or it has expired."
